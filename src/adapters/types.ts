/**
 * The adapter contract. An adapter is the only hollr component that knows a
 * specific agent's shape: it {@link Adapter.normalize | normalizes} that agent's
 * hook payload into a {@link HollrEvent}, and {@link Adapter.wire | wires} the
 * agent's own config so it calls `hollr emit`. Adapters never talk to sinks —
...
/** Result of probing whether an agent is installed and hollr-compatible. */
