 ...
 * with `hollr run -- <cmd>`, so {@link Adapter.wire} is a no-op that only prints
 * usage, and {@link Adapter.detect} always reports installed (it ships with the
 * CLI itself). Events are built by the `hollr run` command, not by
...
const TITLE = "Wrapper";
const USAGE_HINT =
  "wrap any agent with `hollr run -- <cmd> [args...]`; " +
...
export const wrapper: Adapter = {
  ...
  normalize(_raw: unknown, _eventHint: EventName): HollrEvent | null {
    return null;
  },
