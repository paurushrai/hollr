...
import { removeHollrHooks } from "../../src/adapters/hooks.ts";
...
describe("removeHollrHooks", () => {
  it("should_remove_only_matching_entries_and_preserve_foreign_ones", () => {
    ...
    const out = removeHollrHooks(json, ["Stop"], isNested);
  ...
  it("should_be_inert_when_hooks_is_absent_or_not_an_object", () => {
    expect(removeHollrHooks({ a: 1 }, ["Stop"], isNested)).toEqual({ a: 1 });
  });
});
